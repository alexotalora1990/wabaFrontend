<template>
    <div style="height: 1200px; margin: 0 auto; padding: 20px;">
      ...
      <div v-if="storeProdutos.loading" class="overlay">
  ...
  import { useProductosStore } from "../store/productos.js";
  import { useQuasar, Notify } from "quasar";
  
  
  const storeProdutos = useProductosStore();
  const $q = useQuasar();
  ...
  const cargarDatos = async () => {
    try {
      ...
      if (filtro === 'Activos') {
        response = await storeProdutos.ListarActivos();
      } else if (filtro === 'Inactivos') {
        response = await storeProdutos.ListarInactivos();
      } else {
        response = await storeProdutos.ListarTodos();
      }
  ...
  const toggleEstado = async (producto) => {
    try {
      if (producto.estado) {
        await storeProdutos.desactivarProducto(producto._id);
      } else {
        await storeProdutos.activarProducto(producto._id);
      }
  ...
  const guardarDatos = async () => {
    try {
      if (modoEdicion.value) {
        await storeProdutos.actualizarProducto(
          formulario.value._id,
          formulario.value
        );
      } else {
        await storeProdutos.crearProducto(formulario.value);
      }
      await cargarDatos();
